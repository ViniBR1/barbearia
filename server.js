    ...
    try {
        const result = await pool.query(
            `SELECT a.*, c.nome as cliente_nome_completo,
             ...
             LEFT JOIN clientes c ON a.cliente_id = c.id
             WHERE a.barbearia_id = $1 
               AND a.status IN ('aguardando', 'confirmado')
    ...
    try {
        ...
        const result = await pool.query(
            `INSERT INTO agendamentos 
             (barbearia_id, cliente_id, cliente_nome, cliente_cpf, cliente_telefone, 
              servico, plano_id, data_hora, observacao, prioridade, valor_pago, forma_pagamento) 
        ...
        if (!clienteIdInt && clienteTelefone) {
            const novoCliente = await pool.query(
                `INSERT INTO clientes (nome, telefone, cpf, email, senha) 
                 VALUES ($1, $2, $3, $4, $5)
                 RETURNING id`,
                [clienteNome, clienteTelefone, clienteCpf || null, 
                 `${clienteTelefone}@cliente.com`, Buffer.from('123456').toString('base64')]
            );
...
app.put('/api/agendamentos/:id', async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query(
            `UPDATE agendamentos 
             SET status = $1, 
                 finalizado_em = CASE WHEN $1 = 'done' THEN NOW() ELSE finalizado_em END
             WHERE id = $2 
             RETURNING *`,
            [status, id]
        );
...
// ========== ROTAS DE CLIENTES - ADICIONAR CORTE ==========
...
app.post('/api/clientes/usar-corte-gratis', async (req, res) => {
    ...
    try {
        ...
        if ((cliente.rows[0].cortes_gratis || 0) <= 0) {
            return res.status(400).json({ error: 'Cliente não tem cortes grátis disponíveis' });
        }

        const novosCortesGratis = (cliente.rows[0].cortes_gratis || 0) - 1;
    ...
    try {
        ...
        await pool.query(
            `INSERT INTO pagamentos 
             ...
             VALUES ($1, $2, $3, $4, $5)`,
            [clienteIdInt, agendamentoIdInt, response.body.id, valorNumerico, 'pendente']
        );
    ...
    try {
        ...
        if (topic === 'payment') {
            ...
            if (payment.body && payment.body.status === 'approved') {
                ...
                await pool.query(
                    `UPDATE pagamentos SET status = 'aprovado', mp_payment_id = $1 WHERE mp_preference_id = $2`,
                    [id, preferenceId]
                );
